...
{
    // Initialising header
    std::vector<uint8_t> request{0xAB, 0xCD, 0x01, 0x00,
                                 0x00, 0x01, 0x00, 0x00,
                                 0x00, 0x00, 0x00, 0x00};
    ...
    request.push_back(0); // Ending the query

    // Setting the request type to A
    request.push_back(0);
    request.push_back(1);

    // Setting the class to IN
    request.push_back(0);
    request.push_back(1);
    return request;
}
...
std::vector<std::pair<int, std::string>> dnsr_parse(uint8_t *buffer, int msglen)
{
    ...
    // Checking if the message is at least 12 bytes long (DNS header size).
    if (msglen < 12)
    {
    ...
    // Extracting the number of answer records from the DNS header.
    ...
    // Iterating through each answer record.
    for (int i = 0; i < num_answers; ++i)
    {
        answer_ptr += 2; // Ignoring the name section
        int type = (answer_ptr[0] << 8) + answer_ptr[1]; // type - 2 bytes
        answer_ptr += 4;
        int ttl = (answer_ptr[0] << 24) + (answer_ptr[1] << 16) + (answer_ptr[2] << 8) + (answer_ptr[3]); // ttl - 4 bytes
        answer_ptr += 4;

        // Taking the length of the IP address in bytes(IPv4 or IPv6)
        int len = (answer_ptr[0] << 8) + answer_ptr[1];
        answer_ptr += 2;

        // Extracting IP address and storing in response only if it is an A type rr
        std::string r = "";
        while (len--)
        {
...
int main()
{
    printf("Welcome to DNS lookup service v0.0.1 \n");
    ...
    while(1) {
        ...
        // Checking if there is an entry in the cache for the given query and if it is valid at the moment
        bool chk = 1;
        ...
        // Sending the data, dns_request.data() is a pointer to the byte array, dns_request.size() is it's size
        sendto(fd_sock, dns_request.data(), dns_request.size(), 0, (struct sockaddr *)&server, sizeof(server));

        uint8_t buffer[MSG_LEN + 1] = {0};
        socklen_t addrlen = sizeof(server);

        // Receiving the request and parsing it
