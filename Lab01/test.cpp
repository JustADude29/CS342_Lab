...
void solve(){
    char* s = "1 this is my message";
    char* res = base64_encode((unsigned char*)s, strlen(s));
    LOG(res);
    // char* n = new char[res.size()];
    // REP(i,0,res.size()) n[i]=res[i];
    LOG(b64decode(res, strlen(res)));
}